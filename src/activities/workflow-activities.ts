 ...
 */
export async function fetchWorkflowDefinition(workflowId: string): Promise<TemporalWorkflowDefinition> {
  logger.info({ workflow_id: workflowId }, 'Fetching workflow definition from database');
  ...
  try {
    ...
    logger.error({ error, workflow_id: workflowId }, 'Failed to fetch workflow definition');
    throw error;
  }
}
